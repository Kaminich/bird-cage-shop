...
namespace backend_not_clear.Service
{
    public class ProductServices : IProduct
    {
        ...
        public async Task<Product> GetById(string id)
        {
            try
            {
                var found = await this._context.Product
                            ...
                            .Include(x => x.Image)
                            .Include(y => y.OrderDetail)
                                .ThenInclude(z => z.Feedback)
                            .FirstOrDefaultAsync();
                return found;
            }
            catch (Exception ex)
        ...
        public async Task<List<Product>> GetProductsByBirdID(string birdID)
        {
            try
            {
                var products = await _context.Product
                    .Where(p => p.BirdProduct.Any(bp => bp.BirdId.Equals(birdID) && bp.Status))
                    .Include(p => p.Image)
                    .Include(p => p.StyleProduct.Where(sp => sp.Status))
                        .ThenInclude(sp => sp.Style)
                    .Include(p => p.BirdProduct.Where(bp => bp.Status))
                        .ThenInclude(bp => bp.Bird)
                    .Include(p => p.CategoryProduct.Where(cp => cp.Status))
                        .ThenInclude(cp => cp.Category)
                    .Include(p => p.SizeProduct.Where(sp => sp.Status))
                        .ThenInclude(sp => sp.Size)
                    .Include(p => p.ColorProduct.Where(cp => cp.Status))
                        .ThenInclude(cp => cp.Color)
                    .Include(p => p.MaterialProduct.Where(mp => mp.Status))
