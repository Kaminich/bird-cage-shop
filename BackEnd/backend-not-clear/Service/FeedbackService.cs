...
namespace backend_not_clear.Service
{
    public class FeedbackService : IFeedback
    {
        ...
        public async Task<FeedBack> CreateFeedback(CreateFeedback request)
        {
            try
            {
                var product = await this._context.OrderDetail.Where(x => x.OrderId.Equals(request.ProductID)).FirstOrDefaultAsync();
                ...
                product.FeedbackId = feedback.FeedbackId;
                this._context.OrderDetail.Update(product);
